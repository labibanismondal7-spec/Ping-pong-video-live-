...
async function loadFrames() {
  ...
  if (r.success) r.frames.forEach((f) => {
    ...
    const row = document.createElement("div");
    row.className = "data-row";
    row.innerHTML = `${f.imageUrl ? `<img src="${escapeHtml(f.imageUrl)}" alt="" style="width:52px;height:52px;object-fit:contain;border-radius:12px;border:1px solid var(--line);background:rgba(0,0,0,.18);margin-right:12px;" loading="lazy" onerror="this.style.display='none'">` : ""}<div class="data-row-main"><b>${escapeHtml(f.name)}</b><span class="sub">${escapeHtml(f.id)}${f.imageUrl ? " · uploaded PNG" : ""}</span></div>${f.vipOnly ? '<span class="badge badge-warn">VIP only</span>' : ""}`;
    wrap.appendChild(row);
  });
}
