...
// Restore a previously persisted admin session after a clean/restart of the
// Node process. The token/account binding is owned by server.js; this module
// only restores the expiry clocks. Expired records are rejected immediately.
function restore(token, createdAt, lastActiveAt) {
    if (!token) return false;
    const created = Number(createdAt);
    const active = Number(lastActiveAt);
    const now = Date.now();
    if (!Number.isFinite(created) || !Number.isFinite(active)) return false;
    if (now - active > IDLE_TIMEOUT_MS || now - created > ABSOLUTE_TIMEOUT_MS) return false;
    sessionMeta.set(token, { createdAt: created, lastActiveAt: active });
    return true;
}
...
module.exports = { start, restore, get, touch, end, setOnExpire, IDLE_TIMEOUT_MS, ABSOLUTE_TIMEOUT_MS };
