...
function requireUserAuth(req, res, next) {
    ...
    // Production PostgreSQL session fallback. This is intentionally checked
    // after the local/Redis fast paths: normal legacy sessions remain instant,
    // while a production access token can survive instance restarts and
    // load-balancer hops without treating a network failure as logout.
    if (process.env.DATABASE_URL) {
        try {
            const productionRefresh = require("./refreshTokens.js");
            productionRefresh.validate(token).then((session) => {
                if (session && session.user_id) {
                    req.authedMobile = session.user_id;
                    return next();
                }
                // Continue to the Redis path when PostgreSQL has no session.
                if (!sessionStore || !redisClient || !redisClient.isEnabled()) return reject();
                validateTokenCrossInstance(token).then((mobile) => mobile ? (req.authedMobile = mobile, next()) : reject()).catch(() => reject());
            }).catch(() => {
                if (!sessionStore || !redisClient || !redisClient.isEnabled()) return reject();
                validateTokenCrossInstance(token).then((mobile) => mobile ? (req.authedMobile = mobile, next()) : reject()).catch(() => reject());
            });
