...
const uploadPhoto = multer({
    ...
    fileFilter: (req, file, cb) => { if (!/^image\/(jpeg|jpg|png|webp)$/.test(file.mimetype)) return cb(new Error("Only JPG, PNG or WEBP images can be uploaded")); cb(null, true); }
});
...
async function applyChestReward(userId, reward) {
    const found = findUserByUserId(userId);
    if (!found) return;
    if (reward.type === "diamonds") {
        if (productionWallet.enabled()) {
            const wr = await productionWallet.credit(userId, "diamonds", reward.amount, "Treasure Chest reward", "chest-reward", productionWallet.txnId("chest-reward"));
...
app.get("/api/admin/frames/catalog", requireAdmin, requirePermission("frames:view"), (req, res) => {
    res.json({ success: true, frames: frameCatalog.map((f) => ({
        ...
        vipOnly: !!f.vipOnly, createdAt: f.createdAt || null
    })) });
});
...
const productionFrameUpload = multer({storage:multer.memoryStorage(),limits:{fileSize:5*1024*1024},fileFilter:(req,file,cb)=>{if(file.mimetype!=="image/png")return cb(new Error("Only PNG frames can be uploaded"));cb(null,true);}});
        ...
        try {
            ...
            targetList.forEach((targetFound) => {
                ...
                io.to(roomId).emit("video-gift-play", {
                    fromUserId: senderFound.user.userId, fromName: senderFound.user.name,
                    toUserId: targetFound ? targetFound.user.userId : null, toName: targetFound ? targetFound.user.name : null,
                    gift: { id: gift.id, name: gift.name, price: gift.price, quantity: qty, videoUrl: gift.videoUrl, thumbnail: gift.thumbnail, duration: gift.duration }
                });
            });
        } catch (giftErr) {
