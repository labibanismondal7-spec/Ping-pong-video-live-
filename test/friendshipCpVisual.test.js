...
const users = {
  "1111111111": { userId: "u_a", user: { userId: "u_a", name: "Alice", diamonds: 10_000_000 } },
  "2222222222": { userId: "u_b", user: { userId: "u_b", name: "Bob", diamonds: 10_000_000 } }
};
function findUserByUserId(id) {
  return Object.values(users).find((u) => u.userId === id) || null;
}
function saveUsers() {}
function clampDiamondBalance(_uid, val) { return Math.max(0, val); }
...
(async () => {
  ...
  console.log("=== Friendship follows the same adjacency rule (independent of CP) ===");
  {
    ...
    users["3333333333"] = { userId: "u_c", user: { userId: "u_c", name: "Cara", diamonds: 10_000_000 } };
