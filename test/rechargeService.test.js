...
function makeHarness() {
    ...
    function addUser(mobile, userId, diamonds = 0) {
        users[mobile] = { userId, mobile, diamonds, name: "User_" + userId };
        return users[mobile];
    }
...
function makePackage(svc, overrides = {}) {
    const r = svc.createPackage(Object.assign({ priceINR: 100, diamonds: 1000, bonusDiamonds: 100, label: "Popular" }, overrides), "admin_owner");
    return r.package;
}
...
console.log("=== 1. Successful recharge -> correct coins credited ===");
{
    ...
    const before = findUserByUserId("u1").user.diamonds;
    ...
    const after = findUserByUserId("u1").user.diamonds;
    assert(after === before + pkg.diamonds + pkg.bonusDiamonds, "user's coin balance increased by exactly package coins + bonus");
...
console.log("=== 2. Failed / rejected payment -> 0 coins ===");
{
    ...
    assert(findUserByUserId("u2").user.diamonds === 0, "no coins credited on rejection");
}
...
console.log("=== 3. Cancelled payment (user-initiated) -> 0 coins ===");
{
    ...
    assert(findUserByUserId("u3").user.diamonds === 0, "no coins credited on cancellation");
...
console.log("=== 4. Pending payment -> coins not yet credited ===");
{
    ...
    assert(findUserByUserId("u4").user.diamonds === 25, "balance unchanged while a request is still PENDING");
}
...
console.log("=== 5. Admin-approved manual payment -> coins credited ===");
{
    const { svc, addUser, findUserByUserId } = makeHarness();
    enablePayments(svc);
    const pkg = makePackage(svc, { priceINR: 500, diamonds: 5500, bonusDiamonds: 500 });
    ...
    assert(findUserByUserId("u5").user.diamonds === 6000, "admin approval credits package coins + bonus (5500+500=6000)");
}
...
console.log("=== 6. Rejected manual payment -> coins not credited ===");
{
    ...
    assert(findUserByUserId("u6").user.diamonds === 10, "balance unchanged after rejection");
}
...
console.log("=== 7. Same transaction processed twice -> coins credited exactly once (idempotency) ===");
{
    ...
    assert(findUserByUserId("u7").user.diamonds === pkg.diamonds + pkg.bonusDiamonds, "coins credited exactly once, not twice, across two approve calls");
}
...
console.log("=== 12. balance-before/balance-after ledger correctness across multiple recharges ===");
{
    const { svc, addUser } = makeHarness();
    enablePayments(svc);
    const pkg = makePackage(svc, { priceINR: 50, diamonds: 500, bonusDiamonds: 0 });
...
console.log("=== 13. Concurrent/duplicate approve calls on the same transaction never double-credit ===");
{
    const { svc, addUser, findUserByUserId } = makeHarness();
    enablePayments(svc);
    const pkg = makePackage(svc, { priceINR: 100, diamonds: 1000, bonusDiamonds: 0 });
    ...
    assert(findUserByUserId("u13").user.diamonds === 1000, "final balance reflects only a single credit, not five");
}

console.log("=== 14. Package/settings admin surface enforces valid input (defense in depth for route-level validation) ===");
{
    const { svc } = makeHarness();
    const badPrice = svc.createPackage({ priceINR: -5, diamonds: 100 }, "admin_x");
    assert(badPrice.error === "invalid-price", "negative package price rejected");
    const badCoins = svc.createPackage({ priceINR: 10, diamonds: 0 }, "admin_x");
    assert(badCoins.error === "invalid-diamonds", "zero/invalid coin amount rejected");
...
console.log("=== 15. Recharge disabled by admin -> no new requests accepted, existing wallet untouched ===");
{
    ...
    assert(findUserByUserId("u15").user.diamonds === 40, "existing balance is completely untouched");
}
...
console.log("=== 18. Coins remain unchanged while a server-first order is PENDING (before UTR submission) ===");
{
    ...
    assert(findUserByUserId("u18").user.diamonds === 77, "balance is completely untouched by order creation alone");
}
...
console.log("=== 19. UPI deep link / QR payment data for an order carries the order's exact amount and the server orderId as reference ===");
{
    const { svc, addUser } = makeHarness();
    enablePayments(svc);
    const pkg = makePackage(svc, { priceINR: 250, diamonds: 2500, bonusDiamonds: 0 });
...
console.log("=== 21. Server-first flow: submitUtr moves PENDING -> PAYMENT_SUBMITTED, coins still unchanged ===");
{
    ...
    assert(findUserByUserId("u22").user.diamonds === 15, "coins are still unchanged — submitting a UTR never credits by itself");
}
...
console.log("=== 22. Full server-first happy path: PENDING -> PAYMENT_SUBMITTED -> admin approves -> PAID, coins credited exactly once ===");
{
    const { svc, addUser, findUserByUserId } = makeHarness();
    enablePayments(svc);
    const pkg = makePackage(svc, { priceINR: 300, diamonds: 3000, bonusDiamonds: 300 });
    ...
    assert(findUserByUserId("u23").user.diamonds === 3300, "coins credited exactly once (3000+300)");
    ...
    assert(findUserByUserId("u23").user.diamonds === 3300, "balance is unchanged after the redundant approval attempt");
}
...
console.log("=== 23. Server-first order can be rejected by admin -> REJECTED, coins never credited ===");
{
    ...
    assert(findUserByUserId("u24").user.diamonds === 5, "rejected server-first order never credits coins");
}
